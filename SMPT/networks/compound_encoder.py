...
class PlaneEmbedding(nn.Layer):
    """
    Plane Encoder
    """

    def __init__(self, plane_names, embed_dim):
        super(PlaneEmbedding, self).__init__()
        self.plane_names = plane_names

        self.embed_list = nn.LayerList()
        for name in self.plane_names:
            embed = nn.Embedding(
                CompoundKit.get_plane_feature_size(name) + 5,
                embed_dim,
                weight_attr=nn.initializer.XavierUniform())
            self.embed_list.append(embed)
...
class PlaneFloatRBF(nn.Layer):
    """
    Plane Float Encoder using Radial Basis Functions
    """

    def __init__(self, plane_float_names, embed_dim, rbf_params=None):
        super(PlaneFloatRBF, self).__init__()
        self.plane_float_names = plane_float_names

        if rbf_params is None:
            self.rbf_params = {
                'plane_mass': (np.arange(0, 2, 0.1), 10.0),  # (centers, gamma), one plane mass contains four atom
            }
        else:
            self.rbf_params = rbf_params
        ...
        self.linear_list = nn.LayerList()
        self.rbf_list = nn.LayerList()
        for name in self.plane_float_names:
            centers, gamma = self.rbf_params[name]
            rbf = RBF(centers, gamma)
            self.rbf_list.append(rbf)
            linear = nn.Linear(len(centers), embed_dim)
    ...
    def forward(self, plane_float_features):
        """
        Args:
            plane_float_features(dict of tensor): plane float features.
        """
        out_embed = 0
        for i, name in enumerate(self.plane_float_names):
            x = plane_float_features[name]
            rbf_x = self.rbf_list[i](x)
            out_embed += self.linear_list[i](rbf_x)
        return out_embed
...
class DihedralAngleFloatRBF(nn.Layer):
    ...
    def __init__(self, dihedral_angle_float_names, embed_dim, rbf_params=None):
        ...
        if rbf_params is None:
            self.rbf_params = {
                'DihedralAngleGraph_angles': (np.arange(0, np.pi, 0.1), 10.0),  # (centers, gamma)
            }
        else:
            self.rbf_params = rbf_params
    ...
    def forward(self, dihedral_angle_float_features):
        """
        Args:
            dihedral_angle_float_features(dict of tensor): dihedral angle float features.
        """
        out_embed = 0
