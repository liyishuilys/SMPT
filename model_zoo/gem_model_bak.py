...
class GeoGNNModel(nn.Layer):
    ...
    def __init__(self, model_config={}):
        ...
        print('[GeoGNNModel] plane_names:%s' % str(self.plane_names))
        print('[GeoGNNModel] plane_float_names:%s' % str(self.plane_float_names))
        print('[GeoGNNModel] dihedral_angle_float_names:%s' % str(self.dihedral_angle_float_names))
    ...
    def forward(self, atom_bond_graph, bond_angle_graph, dihedral_angle_graph):
        ...
        plane_hidden_list = [plane_hidden]
        ...
        for layer_id in range(self.layer_num):
            ...
            cur_plane_hidden = cur_plane_hidden + self.plane_float_rbf_list[layer_id](dihedral_angle_graph.node_feat)
        ...
        # print("[GeoGNNModel] forward node_hidden_list: ", node_hidden_list)  # shape=[31, 32],
        # print("[GeoGNNModel] forward edge_hidden_list: ", edge_hidden_list)  # shape=[97, 32]
        node_repr = node_hidden_list[-1]
        edge_repr = edge_hidden_list[-1]
        plane_repr = plane_hidden_list[-1]
        graph_repr = self.graph_pool(atom_bond_graph, node_repr)
        # print("[GeoGNNModel] forward graph_repr: ", graph_repr)  # shape=[2, 32]
...
class GeoPredModel(nn.Layer):
    ...
    def _get_Cm_loss(self, feed_dict, node_repr):
        ...
        # print("[GeoPredModel] _get_Cm_loss masked_node_repr:", masked_node_repr)  # shape=[4, 32]
        logits = self.Cm_linear(masked_node_repr)
        # print("[GeoPredModel] _get_Cm_loss logits:", logits)  # shape=[4, 2403]
        loss = self.Cm_loss(logits, feed_dict['Cm_context_id'])
        # print("[GeoPredModel] _get_Cm_loss loss:", loss)  # shape=[1]
        return loss
